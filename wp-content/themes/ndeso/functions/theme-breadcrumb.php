...
/**
 * Breadcrumb tema Ndeso dari dimox breadcrumb
 *
...
function dimox_breadcrumbs() {
	...
	/* === OPTIONS === */
	$text['home']	 = __('Home', 'utravel'); // text for the 'Home' link
	$text['category'] = __('Kategori "%s"', 'utravel'); // text for a category page
	$text['search']   = __('Pencarian : "%s"', 'utravel'); // text for a search results page
	$text['tag']	  = __('Tag "%s"', 'utravel'); // text for a tag page
	$text['author']   = __('Tulisan dari %s', 'utravel'); // text for an author page
	$text['404']	  = __('Error 404', 'utravel'); // text for the 404 page
	...
	if ( is_home() || is_front_page() || (empty($post)) ) {
		...
		if ($show_on_home == 1)
			echo '<div class="breadcrumbs">Anda ada di : <a href="' . $home_link . '">' . $text['home'] . '</a></div>';
